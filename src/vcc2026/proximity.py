...
Everything else here transfers a measured transcriptional response from one cell
...
tethering a KRAB domain, which deposits H3K9me3 that spreads tens of kilobases
from the guide, and the spreading is a property of the *reagent* and of local
chromatin, not of which cell line it is done in.  So the genes sitting next to a
target go down when it is knocked down, and gene coordinates are the same in
every cell line.
...
|------------|-------|----------------|---------|-------------|
| < 100 kb   |   119 | 0.269          | 0.647   | -0.363      |
| 100-500 kb |   421 | 0.131          | 0.532   | -0.025      |
| 0.5-1 Mb   |   474 | 0.095          | 0.508   |  0.003      |
| other      |  602k | 0.096          | 0.498   | -0.001      |

The background rate is 0.096, so within 100 kb a gene is 2.8x more likely to be
called and goes down about two times in three.  It is a small set -- a median of
six genes per perturbation within 500 kb -- which is why it cannot move the DE
members, whose denominators run to a thousand.  It moves `pds_cosine`, which
ranks predictions against each other by cosine: these coordinates belong to one
target and nothing else, and carry a large signed magnitude.

`scripts/proximity_effect.py` reproduces the table.
"""
